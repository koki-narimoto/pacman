...
class MultiPacmanAgent(MultiAgentSearchAgent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        eval_value = 0
        legalMoves = successorGameState.getLegalPacmanActions(self.index)
        if len(newFood.asList()):
            fooddist = util.manhattanDistance(newPos, newFood.asList()[0])
        else:
            fooddist = 0
    ...
    def minimax(self, currentGameState, action):
        """
        Design a better evaluation function here.
        ...
        for moves in legalMoves:
            # after pacman 1 takes a move this is where the ghost  min will be located
